...
> **注意：** 如果之后在设置中切换到 FunASR 或 MLX-Whisper，VoiceText 需要下载模型（根据模型不同，从 ~75 MB 到 ~1.6 GB 不等）。下载期间：
>
...
>
> **提示：** 如果下载失败或中断，删除缓存目录（FunASR 为 `~/.cache/modelscope/`，MLX-Whisper 为 `~/.cache/huggingface/`）后重启 VoiceText 重试。
...
VoiceText 会在内存中保留最近 **10 条** 预览结果（应用重启后清除）。这让你可以回溯之前的转录而无需重新录音。
...
当使用支持流式识别的 ASR 后端（目前为 Apple Speech）时，VoiceText 会在录音过程中显示 **实时转录悬浮窗**。你说话的同时即可看到文字逐步出现，在松开快捷键之前就能获得即时反馈。
...
当对话历史超过 **20,000 条记录** 时，VoiceText 会自动将较旧的记录归档到 `~/.config/VoiceText/conversation_history_archives/YYYY-MM.jsonl` 月度文件中。主历史文件保留最近 20,000 条记录以确保快速访问，归档记录仍可通过历史浏览器搜索和查看。
...
在通用标签中，你可以设置 **自定义配置目录** 将 VoiceText 配置文件存储在自选位置（例如同步文件夹）。更改目录后，VoiceText 会提示你重启以使更改生效。

#### 脚本开关

通用标签中包含 **脚本** 开关，用于启用或禁用脚本/插件系统。启用后，VoiceText 会从配置的脚本目录加载并执行 Lua 脚本。详见 [脚本系统文档](scripting-zh.md)。
...
默认位置：`~/.config/VoiceText/config.json`
...
- **编写脚本** — 使用 Lua 插件扩展 VoiceText 的自定义快捷键功能（详见 [脚本系统文档](scripting-zh.md)）

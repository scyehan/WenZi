...
class TestHotkeyAPI:
    ...
    @patch("voicetext.scripting.api.hotkey.AppHelper", create=True)
    def test_leader_subkey_executes(self, mock_helper):
        _, api = self._make_api()
        api._on_press("cmd_r")
    ...
    @patch("voicetext.scripting.api.hotkey.AppHelper", create=True)
    def test_leader_release_deactivates(self, mock_helper):
        _, api = self._make_api()
        api._on_press("cmd_r")
        api._on_press("w")
    ...
    @patch("voicetext.scripting.api.hotkey.AppHelper", create=True)
    def test_unmatched_subkey_still_swallowed(self, mock_helper):
        _, api = self._make_api()
        api._on_press("cmd_r")
    ...
    def test_execute_mapping_app(self):
        _, api = self._make_api()
        with patch("voicetext.scripting.api.app.AppAPI.launch", return_value=True) as mock_launch:
            api._execute_mapping(LeaderMapping(key="w", app="WeChat"))
            mock_launch.assert_called_once_with("WeChat")

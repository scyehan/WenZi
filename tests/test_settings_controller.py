...
from voicetext.settings_controller import SettingsController
...
class TestHotkeyToggle:
    @patch("voicetext.settings_controller.save_config")
...
class TestSttSelect:
    ...
    def test_busy_shows_alert(self, ctrl, mock_app):
        mock_app._busy = True
        with patch("voicetext.settings_controller.topmost_alert") as mock_alert:
            with patch("voicetext.settings_controller.restore_accessory"):
...
class TestOnOpenSettings:
    def test_shows_panel_with_state_and_callbacks(self, ctrl, mock_app):
        with patch("voicetext.vocabulary.get_vocab_entry_count", return_value=5):
            with patch("voicetext.settings_controller.PRESETS", []):
                with patch("voicetext.settings_controller.build_remote_asr_models", return_value=[]):

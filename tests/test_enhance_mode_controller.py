...
class TestOnEnhanceModeSelect:
    @patch("voicetext.enhance_mode_controller.save_config")
...
class TestUpdateVocabTitle:
    @patch("voicetext.enhance_mode_controller.get_vocab_entry_count", return_value=42)
    ...
    @patch("voicetext.enhance_mode_controller.get_vocab_entry_count", return_value=0)
...
class TestOnVocabBuild:
    def test_no_enhancer_shows_alert(self, ctrl, mock_app):
        mock_app._enhancer = None
        with patch("voicetext.enhance_mode_controller.topmost_alert") as mock_alert:

"""
Auto-update module for WenZi macOS app.
...
class AppUpdater:
    ...
    def _run(self):
        ...
        try:
            ...
            # Find WenZi.app in mounted volume
    ...
    def _download_dmg(self, dest: Path):
        ...
        req = urllib.request.Request(
            self.dmg_url,
            headers={"User-Agent": "WenZi-Updater"},
        )
    ...
    def perform_swap_and_relaunch() -> bool:
        ...
        pid = os.getpid()
        q_app = shlex.quote(str(app_path))
        q_staged = shlex.quote(str(staged_app))
        q_backup = shlex.quote(str(app_path) + ".bak")

        # Shell script that waits for exit, then swaps app bundles with
        # rollback safety: rename current → .bak, move staged → current.
...
sleep 0.5
mv {q_app} {q_backup} && mv {q_staged} {q_app}
if [ $? -ne 0 ]; then
    mv {q_backup} {q_app} 2>/dev/null
    exit 1
fi
rm -rf {q_backup}
xattr -rd com.apple.quarantine {q_app} 2>/dev/null
open {q_app}
"""
        subprocess.Popen(
            ["bash", "-c", script],
